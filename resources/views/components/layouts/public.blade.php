...
<body class="font-sans antialiased text-secondary bg-bg-light selection:bg-primary/30">
    ...
    >
        ...
        <div class="flex-1 flex items-center justify-end gap-4 sm:gap-6">
            @if($whatsappNumber)
                <a href="https://wa.me/{{ $whatsappNumber }}{{ $whatsappGeneralTemplate ? '?text=' . urlencode($whatsappGeneralTemplate) : '' }}" target="_blank"
    ...
    <div x-show="mobileMenuOpen" 
            ...
            class="fixed inset-0 z-[200] bg-white md:hidden shadow-2xl flex flex-col" 
            style="display: none;"
            x-cloak>
        ...
        <div class="px-6 sm:px-8 py-5 sm:py-6 flex items-center justify-between border-b border-gray-100 shrink-0">
            <a href="{{ route('home') }}" @click="mobileMenuOpen = false" class="flex items-center gap-2 group relative shrink-0" wire:navigate title="{{ $siteName }}">
                @if($logoImage)
                    <img src="{{ Storage::url($logoImage) }}" alt="{{ $siteName }}" class="h-16 w-auto sm:h-20 object-contain" />
                @else
        ...
        <div class="flex flex-col h-full pb-12 px-8 overflow-y-auto">
            ...
            @if($whatsappNumber)
                <div class="mt-10 mx-auto w-full max-w-xs">
                    <a href="https://wa.me/{{ $whatsappNumber }}{{ $whatsappGeneralTemplate ? '?text=' . urlencode($whatsappGeneralTemplate) : '' }}" target="_blank" class="block w-full py-4 bg-secondary text-white text-center rounded-full text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-primary transition-colors shadow-lg">
                        {{ __('Inquire Now') }}
                    </a>
                </div>
            @endif
    ...
    {{-- Footer --}}
    <footer class="bg-white pt-16 md:pt-24 pb-8 px-6 sm:px-8 border-t border-gray-100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-16 md:mb-20">
                <div class="sm:col-span-2 lg:col-span-4 lg:pr-10">
                    <div class="mb-3 inline-block">
                        @if($logoImage)
                            <img src="{{ Storage::url($logoImage) }}" alt="{{ $siteName }}" class="h-24 md:h-32 w-auto object-contain" />
                        @else
                ...
                <div class="lg:col-span-2">
                    <h4 class="text-secondary font-bold uppercase tracking-[0.15em] text-xs mb-6">{{ __('Get In Touch') }}</h4>
                    <ul class="space-y-4">
                        @if($whatsappNumber)
                            <li>
                                <a class="text-secondary/60 hover:text-primary transition-colors text-sm font-medium flex items-center gap-3 group" href="https://wa.me/{{ $whatsappNumber }}{{ $whatsappGeneralTemplate ? '?text=' . urlencode($whatsappGeneralTemplate) : '' }}" target="_blank">
                                    <div class="w-9 h-9 rounded-full bg-secondary/5 flex items-center justify-center group-hover:bg-primary/10 transition-colors">
                                        <i class="material-icons text-lg text-primary">phone</i>
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="text-[10px] uppercase tracking-wider text-secondary/40 font-bold leading-none mb-1">{{ __('WhatsApp') }}</span>
                                        <span class="text-secondary group-hover:text-primary transition-colors decoration-primary/30">{{ $whatsappNumber }}</span>
                                    </div>
                                </a>
                            </li>
                        @endif
                        @php $adminEmail = \App\Models\Setting::get('admin_email'); @endphp
                        @if($adminEmail)
                            <li>
                                <a class="text-secondary/60 hover:text-primary transition-colors text-sm font-medium flex items-center gap-3 group" href="mailto:{{ $adminEmail }}">
                                    <div class="w-9 h-9 rounded-full bg-secondary/5 flex items-center justify-center group-hover:bg-primary/10 transition-colors">
                                        <i class="material-icons text-lg text-primary">email</i>
                                    </div>
                                    <div class="flex flex-col">
                ...
                <div class="lg:col-span-2">
                    <h4 class="text-secondary font-bold uppercase tracking-[0.15em] text-xs mb-6">{{ __('Social Media') }}</h4>
                    <div class="flex flex-wrap gap-3">
                    @if($socialInstagram)
                        <a class="w-10 h-10 rounded-full border border-secondary/10 flex items-center justify-center text-secondary hover:bg-primary hover:text-white hover:border-primary transition-all" href="{{ $socialInstagram }}" target="_blank">
                    ...
                    @endif
                    @if($socialFacebook)
                        <a class="w-10 h-10 rounded-full border border-secondary/10 flex items-center justify-center text-secondary hover:bg-primary hover:text-white hover:border-primary transition-all" href="{{ $socialFacebook }}" target="_blank">
                    ...
                    @endif
                    @if($socialTwitter)
                        <a class="w-10 h-10 rounded-full border border-secondary/10 flex items-center justify-center text-secondary hover:bg-primary hover:text-white hover:border-primary transition-all" href="{{ $socialTwitter }}" target="_blank">
                    ...
                    @endif
                    @if($socialYoutube)
                        <a class="w-10 h-10 rounded-full border border-secondary/10 flex items-center justify-center text-secondary hover:bg-primary hover:text-white hover:border-primary transition-all" href="{{ $socialYoutube }}" target="_blank">
                    ...
                    @endif
                    @if($socialTiktok)
                        <a class="w-10 h-10 rounded-full border border-secondary/10 flex items-center justify-center text-secondary hover:bg-primary hover:text-white hover:border-primary transition-all" href="{{ $socialTiktok }}" target="_blank">
